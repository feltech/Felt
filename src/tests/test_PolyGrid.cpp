...
SCENARIO("PolyGrid")
{
	...
	WHEN("poly_cubes")
	{
		...
		CHECK(poly.get(Vec3i(0,0,0)).vtx().size() == 30);
		CHECK(poly.get(Vec3i(0,0,0)).spx().size() == 56);
		CHECK(
...
//		INFO(stringify_grid_slice(surface.isogrid()));
		...
		CHECK(total_vtx == poly_single.vtx().size() + 12 + 3 - 3);
		...
		CHECK(poly.get(Vec3i(-1,0,0)).vtx().size() == 5);
		CHECK(poly.get(Vec3i(0,-1,0)).vtx().size() == 5);
		CHECK(poly.get(Vec3i(0,0,-1)).vtx().size() == 5);
...
//		INFO(stringify_grid_slice(surface.isogrid()));
		...
		// partition.
		CHECK(total_vtx == poly_single.vtx().size() + 8 + 2 - 2);
...
//		INFO(stringify_grid_slice(surface.isogrid()));
		...
		CHECK(surface.isogrid().children().data().size() == 1u);
		...
		CHECK(num_spxs_after == num_spxs_before);
		CHECK(num_vtxs_after == num_vtxs_before);
	}

	WHEN("poly_cubes_expand_contract_single_partition")
	{
		// ==== Setup ====
...
//		INFO(stringify_grid_slice(surface.isogrid()));
		...
		CHECK(surface.isogrid().children().data().size() == 1);
	...
	WHEN("poly_cubes_expand_contract_across_partition")
	{
		// ==== Setup ====
...
//		INFO(stringify_grid_slice(surface.isogrid()));
	...
	WHEN("poly_cubes_in_partition_hosting_neighbours_polys")
	{
		// ==== Setup ====
